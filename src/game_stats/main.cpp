...
template <> struct hash<chess::Square> {
  size_t operator()(const chess::Square &sq) const { return sq.index(); }
};
} // namespace std
...
using StringUint32Map =
    unordered_map<string, uint32_t, TransparentStringHash, equal_to<>>;

using StringUint8Map =
    unordered_map<string, uint8_t, TransparentStringHash, equal_to<>>;
...
vector<uint8_t> computeGameAttackedPieces(const GameData &game,
                                          Color colorFilter) {
...
vector<int8_t> computeGameMaterialBalances(const GameData &game,
                                           Color colorFilter) {
...
vector<int8_t> computeGameCenterControl(const GameData &game,
                                        Color colorFilter) {
  vector<int8_t> center_control;

  vector<Square> center_sq = {Square::SQ_D4, Square::SQ_E4, Square::SQ_D5,
                              Square::SQ_E5};
  ...
  for (size_t i = 0; i < game.moves.size(); ++i) {
    ...
    for (const Square &sq : center_sq) {
      ...
      if (piece != Piece::NONE && piece.color() != curr.sideToMove() &&
...
vector<int8_t> computeGameExtendedCenterControl(const GameData &game,
                                                Color colorFilter) {
...
StringUint8Map computeGamePawnStruct(const GameData &game, Color colorFilter) {
  ...
  auto pawnOnFile = [&curr](Color side, uint8_t file) {
    uint8_t c = 0;
      ...
      if (cnt >= 1) {
        ...
        for (uint8_t ef = std::max(0, f - 1); ef <= std::min(7, f + 1); ++ef) {
          for (uint8_t r = 0; r < 8; ++r) {
            ...
            if (epc != Piece::NONE && epc.type() == PAWN &&
                epc.color() != side) {
              passed = false;
              break;
            }
  ...
  return {{"isolated", isolated_pawns},
          {"doubled", doubled_pawns},
          {"passed", passed_pawns},
          {"fianchetto", fianchetto_pawns}};
}
